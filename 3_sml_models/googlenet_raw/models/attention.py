...
class Attention(nn.Module):
    ...
    def __init__(self, block_num, class_num=5):
    ...
    def forward(self, x):
        x = self.pre_conv(x)
        #print(x.shape,'self.pre_conv')
        x = self.stage1(x)
        #print(x.shape,'self.stage1')
        x = self.stage2(x)
        #print(x.shape,'self.stage2')
        x = self.stage3(x)
        #print(x.shape,'self.stage3')
        x = self.stage4(x)
        #print(x.shape,'self.stage4')
        x = self.avg(x)
        #print(x.shape,'self.avg')
        x = x.view(x.size(0), -1)
